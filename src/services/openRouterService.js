...
class OpenRouterService {
  constructor() {
    ...
    if (!this.isEnabled) {
      console.warn('🔑 OpenRouter API key not configured - secret feature disabled');
    } else {
  ...
  async sendChatCompletion(messages, options = {}) {
    ...
    const defaultOptions = {
      model: 'google/gemini-flash-1.5', // Default non-OpenAI model
      temperature: 0.7,
      max_tokens: 4000, // Increased for Gemini Flash 1.5
      stream: false,
      ...options
    };

    try {
      console.log('🚀 Sending OpenRouter API request:', {
