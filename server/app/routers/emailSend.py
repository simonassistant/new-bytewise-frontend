...
def send_email():
    try:
        ...
        course_title = data.get(
            "course_title", "HKBU LANG 0036 Learning Session Report"
        )
...
def report_to_markdown(
    ...
    if student_number:
        md += f"**Student Number:** {student_number}  \n"
    if section_number:
        md += f"**Section:** {section_number}  \n"

    md += "\n---\n"
    if report_info:
        md += "## Course Information and Student Background"
        md += report_info
...
def report_history_to_html(
    ...
    try:
        ...
        if student_email:
            html_content += (
                f"""<p><strong>Student Email:</strong> {student_email}</p>"""
            )
        if student_number:
            html_content += (
                f"""<p><strong>Student Number:</strong> {student_number}</p>"""
            )
        if section_number:
            html_content += f"""<p><strong>Section:</strong> {section_number}</p>"""
        html_content += "</div>"
